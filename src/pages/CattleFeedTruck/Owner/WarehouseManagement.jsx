...
const WarehouseManagement = () => {
    ...
    const fetchWarehouses = async () => {
        try {
            ...
            const response = await axios.get('http://15.206.212.140:5000/api/cattle-feed-truck/warehouses', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingWarehouse) {
                await axios.put(
                    `http://15.206.212.140:5000/api/cattle-feed-truck/warehouses/${editingWarehouse._id}`,
                    formData,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
            } else {
                await axios.post('http://15.206.212.140:5000/api/cattle-feed-truck/warehouses', formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://15.206.212.140:5000/api/cattle-feed-truck/warehouses/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchWarehouses();
        } catch (error) {
