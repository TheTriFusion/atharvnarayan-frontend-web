...
const FeedProductManagement = () => {
    ...
    const fetchProducts = async () => {
        try {
            ...
            const response = await axios.get('http://43.204.211.69:5000/api/cattle-feed-truck/feed-products', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingProduct) {
                await axios.put(
                    `http://43.204.211.69:5000/api/cattle-feed-truck/feed-products/${editingProduct._id}`,
                    formData,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
            } else {
                await axios.post('http://43.204.211.69:5000/api/cattle-feed-truck/feed-products', formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://43.204.211.69:5000/api/cattle-feed-truck/feed-products/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchProducts();
        } catch (error) {
