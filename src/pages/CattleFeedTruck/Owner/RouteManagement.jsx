...
const RouteManagement = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [routesRes, pointsRes] = await Promise.all([
                axios.get('http://localhost:5000/api/cattle-feed-truck/routes', { headers: { Authorization: `Bearer ${token}` } }),
                axios.get('http://localhost:5000/api/cattle-feed-truck/delivery-points', { headers: { Authorization: `Bearer ${token}` } }),
            ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            if (editingRoute) {
                await axios.put(`http://localhost:5000/api/cattle-feed-truck/routes/${editingRoute._id}`, formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            } else {
                await axios.post('http://localhost:5000/api/cattle-feed-truck/routes', formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://localhost:5000/api/cattle-feed-truck/routes/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchData();
        } catch (error) {
