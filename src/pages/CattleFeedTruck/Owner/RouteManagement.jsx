...
const RouteManagement = () => {
    ...
    const fetchData = async () => {
        try {
            ...
            const [routesRes, pointsRes] = await Promise.all([
                axios.get('https://api.thetrifusion.in/api/cattle-feed-truck/routes', { headers: { Authorization: `Bearer ${token}` } }),
                axios.get('https://api.thetrifusion.in/api/cattle-feed-truck/delivery-points', { headers: { Authorization: `Bearer ${token}` } }),
            ]);
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            const token = localStorage.getItem('token');
            if (editingRoute) {
                await axios.put(`https://api.thetrifusion.in/api/cattle-feed-truck/routes/${editingRoute._id}`, formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            } else {
                await axios.post('https://api.thetrifusion.in/api/cattle-feed-truck/routes', formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`https://api.thetrifusion.in/api/cattle-feed-truck/routes/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchData();
        } catch (error) {
