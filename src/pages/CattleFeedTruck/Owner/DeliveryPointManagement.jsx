...
const DeliveryPointManagement = () => {
    ...
    const fetchDeliveryPoints = async () => {
        try {
            ...
            const response = await axios.get('https://api.thetrifusion.in/api/cattle-feed-truck/delivery-points', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingPoint) {
                await axios.put(
                    `https://api.thetrifusion.in/api/cattle-feed-truck/delivery-points/${editingPoint._id}`,
                    formData,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
            } else {
                await axios.post('https://api.thetrifusion.in/api/cattle-feed-truck/delivery-points', formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`https://api.thetrifusion.in/api/cattle-feed-truck/delivery-points/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchDeliveryPoints();
        } catch (error) {
