...
const DeliveryPointManagement = () => {
    ...
    const fetchDeliveryPoints = async () => {
        try {
            ...
            const response = await axios.get('http://43.204.211.69:5000/api/cattle-feed-truck/delivery-points', {
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (editingPoint) {
                await axios.put(
                    `http://43.204.211.69:5000/api/cattle-feed-truck/delivery-points/${editingPoint._id}`,
                    formData,
                    { headers: { Authorization: `Bearer ${token}` } }
                );
            } else {
                await axios.post('http://43.204.211.69:5000/api/cattle-feed-truck/delivery-points', formData, {
                    headers: { Authorization: `Bearer ${token}` },
                });
            }
            setShowModal(false);
    ...
    const handleDelete = async (id) => {
        ...
        try {
            const token = localStorage.getItem('token');
            await axios.delete(`http://43.204.211.69:5000/api/cattle-feed-truck/delivery-points/${id}`, {
                headers: { Authorization: `Bearer ${token}` },
            });
            fetchDeliveryPoints();
        } catch (error) {
